    ...
    super({
      jwtFromRequest: ExtractJwt.fromExtractors([
        (request) => {
          ...
          if (!keyCheckAuthentication || (!_getKeyAuthentication && !cookie.user_refresh_token)) {
            throw new UnauthorizedException()
          }

          let token = ''
          if (cookie.user_refresh_token) {
            token = cookie.user_refresh_token.replace('Bearer ', '')?.trim()
          } else {
